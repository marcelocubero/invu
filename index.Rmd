...
cuadrantes <- st_read("/vsicurl/https://marcelocubero.github.io/capas_TFG/cuadrantes_cr.geojson",
         quiet = TRUE)
...
numericRangeInput(
 ...
 filtrarRegistros <-  reactive({
  cuadrantes_f <-
  cuadrantes  %>%
   dplyr::select( area_km2, aprob, canton, distrito, nomb_zona)
  
  cuadrantes_f <-
    cuadrantes_f  %>%
    
    filter(
    area_km2 >= input$area_km2[1] &
    area_km2 <= input$area_km2[2]
  )
